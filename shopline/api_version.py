...
class ApiVersion(object):
    ...
    def define_known_versions(cls):
        ...
        cls.define_version(Release("v20220901"))
        cls.define_version(Release("v20240301"))
        cls.define_version(Release("v20241201"))
        cls.define_version(Release("v20250301"))
...
class Release(ApiVersion):
    FORMAT = re.compile(r"v\d{8}$")
    API_PREFIX = "/admin/openapi"

    def __init__(self, version_number):
        if not self.FORMAT.match(version_number):
            raise InvalidVersionError
        self._name = version_number
        self._numeric_version = int(version_number.replace("v", ""))
